...
mob/living/simple_mob/mobject
	...
	var/morphitem = /obj/item/weapon/tool/wrench //Since its the most used item for anchoring this wil be default
	var/powertoggle //off by default
	var/altpowertoggle = 1 //on by default, turn off/on on click by hand

mob/living/simple_mob/mobject/New()
	..()
	icon = animal_icon
	icon_living = animal_icon_state

mob/living/simple_mob/mobject/attack_hand(mob/user as mob) //Togglecode
	if(obj_on)
		if(altpowertoggle)
			if(on)
				on = !on
...
mob/living/simple_mob/mobject/attackby(obj/item/I, mob/user) //Togglecode
...
mob/living/simple_mob/mobject/Life()
...
mob/living/simple_mob/mobject/process() //proccess performed instead of Life in OBJ form.
	if(!on)
		return //A way to disable process() //for PandL this will also disable Life()

mob/living/simple_mob/mobject/death()
	..()
	icon = animal_icon
	update_icons()
...
mob/living/simple_mob/mobject/metamorphicfennec
